...
const MultiStepForm = () => {
  ...
  const isStepComplete = (stepNumber) => {
    switch (stepNumber) {
      case 1:
        return formData.course && formData.semester && formData.examType  && formData.rollno && formData.dateOfBirth;
      case 2:
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-100 py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
      <div className="max-w-2xl w-full bg-white rounded-xl shadow-2xl overflow-hidden transform transition-all duration-300 hover:scale-[1.01]">
        <div className="px-8 py-6 bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
          <h1 className="text-3xl font-bold text-center">Hostel Registration Form</h1>
          <p className="text-center text-indigo-100 mt-2">Complete all steps to submit your application</p>
        </div>
        ...
        <div className="p-8">
          ...
          <div className="mb-8 mt-6">
            {step === 1 && <PersonalInformation formData={formData} handleChange={handleChange} />}
            {step === 2 && <EmailMobileVerification formData={formData} handleChange={handleChange} />}
            {step === 3 && <HostelSelection formData={formData} handleChange={handleChange} />}
